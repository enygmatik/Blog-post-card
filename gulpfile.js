...
(function(scope){
  ...
  gulp.task('p-scripts:package:now', function(){
    var jsBundles = [distDir + '/scripts/post-card.min.js'];
  ...
  var scriptsBrowserify = function(env){
    var jsBundles = ['app/scripts/post-card.js'];
...
(function(scope){
  ...
  gulp.task('vulcanize:inline:now',function(){
    return gulp.src(distDir + '/post-card.html', {base: distDir + '/'})
      .pipe($.vulcanize({
  ...
  var vulcanizeWithExternalScriptsAndStyles = function(vulcanizeOptions){
    return gulp.src(tmpDir + '/post-card.html')
